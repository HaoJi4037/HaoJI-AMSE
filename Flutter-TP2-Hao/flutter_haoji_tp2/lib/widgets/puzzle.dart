...
class PuzzleState extends State<Puzzle> {
  ...
  void didUpdateWidget(Puzzle oldWidget) {
    ...
    if (oldWidget.image != widget.image ||
        oldWidget.itemX != widget.itemX ||
        oldWidget.itemY != widget.itemY) {
      cropImage(widget.image, widget.itemSize, widget.itemX, widget.itemY);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Listener(
      ...
      onPointerUp: (details) {
        if (status == 1) {
          int endX = ((endOPosition.dx - deltOffset.dx) / widget.itemSize.width)
              .round();
          int endY =
              ((endOPosition.dy - deltOffset.dy) / widget.itemSize.height)
                  .round();
          ...
          if (endX - x == 0 && endY - y == 0) {
            ...
            if (next < index) {
              tempKey.currentState
                  .startAnimation(Duration(milliseconds: 200), offsets[next]);
              (widgets[next].key as GlobalKey<ImageItemState>)
                  .currentState
              ...
              tempKey.currentState
                  .startAnimation(Duration(milliseconds: 200), offsets[next]);
              (widgets[next - 1].key as GlobalKey<ImageItemState>)
                  .currentState
                  .startAnimation(Duration(milliseconds: 200), offsets[index]);
  ...
  cropImage(ui.Image image, Size size, int itemX, int itemY) {
    ...
    for (int i = 0; i < itemY; i++) {
      for (int j = 0; j < itemX; j++) {
        ...
        Rect rawRect =
            Rect.fromLTWH(j * itemWidth, i * itemHeight, itemWidth, itemHeight);
  ...
  startWithRandomWidgets() {
    ...
    for (int i = 0; i < offsets.length; i++) {
      (widgets[i].key as GlobalKey<ImageItemState>)
          .currentState
          .updateOffset(offsets[i]);
    }
    setState(() {
      status = 1;
    });
  ...
  check() {
    ...
    status = 0;
    MyNotification("End").dispatch(context);
    //   print("游戏结束");
  }
}
